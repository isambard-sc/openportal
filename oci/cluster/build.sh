...
out=$(cargo build --package op-cluster --target=x86_64-unknown-linux-musl --message-format=json ${@-})
cp "$(artifact_path "${out}" "op-cluster")" oci/cluster

cd oci/cluster

version=$(./op-cluster --version | tail -n1 | cut -d' ' -f 2)
image_id=$(
  podman build . --tag=op-cluster:latest --tag=op-cluster:"${version}" \
...
rm op-cluster
echo "Built op-cluster image:" 1>&2
echo "${image_id}"
