...
out=$(cargo build --package op-platform --target=x86_64-unknown-linux-musl --message-format=json ${@-})
cp "$(artifact_path "${out}" "op-platform")" oci/platform

cd oci/platform

version=$(./op-platform --version | tail -n1 | cut -d' ' -f 2)
image_id=$(
  podman build . --tag=op-platform:latest --tag=op-platform:"${version}" \
...
rm op-platform
echo "Built op-platform image:" 1>&2
echo "${image_id}"
