...
export PORTAL_AWARDING_PORTALS=award     # who may make awards here
...
With the application running and an awarding portal called `award` configured,
here is the whole life of an award.

**1. `award` creates it, on a particular resource.** It addresses
`award.portal.cluster1`, and that last element is a *virtual agent* on this
...
curl -X POST localhost:8080/awards/cluster1/myaward1.award/approve \
     -H 'content-type: application/json' \
     -d '{"local_project_id": "myproject1.portal", "reason": "approved by the panel"}'
```
...
`award` knows this award as `myaward1.award`; we now know the project we made for
it as `myproject1.portal`. Neither side could guess the other's name, so approval is
where ours gets decided.

**3. It goes live by itself, and `award` learns our name for it.** Nothing is
pushed back. `award` is already re-sending `create_award` every cycle, so the
...
produces figures for `myproject1.portal` and has never heard of `myaward1.award`, so
that is what it posts against:

```bash
curl -X PUT localhost:8080/projects/myproject1.portal/usage \
     -H 'content-type: application/json' \
...
Note the endpoints under `/awards` are keyed on `award`'s identifier and this one
on ours. That is not an inconsistency — it is the two namespaces, and the
mapping is what joins them.

**5. `award` collects them, and gets an answer in its own namespace.** It asks
`award.portal.cluster1 get_usage_report myaward1.award`. The figures were recorded against
`myproject1.portal`, so `build_usage_report` assembles the report in our namespace
and then `remap_project`s it into theirs — `alice.myproject1.portal` becomes
`alice.myaward1.award`, and the member's email is untouched because it is the
...
`award.portal.cluster2 get_usage_report myaward1.award` and the answer is an
